...
- Mets les termes importants en **gras**`;
    ...
    // ==========================================
    //  9. GÉNÉRATEUR PESTEL INTERACTIF
    // ==========================================

    const pestelEvents = [
        { id: 'pe1', text: "Nouvelle loi sur le télétravail", cat: 'L', influence: { P: 1, E: 2, S: 5, T: 4, ENV: 3, L: 5 } },
        { id: 'pe2', text: "Pénurie de semi-conducteurs", cat: 'T', influence: { P: 2, E: 5, S: 1, T: 5, ENV: 1, L: 1 } },
        { id: 'pe3', text: "Hausse du prix de l'électricité", cat: 'E', influence: { P: 3, E: 5, S: 2, T: 1, ENV: 4, L: 1 } },
        { id: 'pe4', text: "RGPD (Règlementation Données)", cat: 'L', influence: { P: 4, E: 2, S: 3, T: 4, ENV: 1, L: 5 } },
        { id: 'pe5', text: "Essor de l'IA Générative", cat: 'T', influence: { P: 2, E: 4, S: 4, T: 5, ENV: 2, L: 3 } },
        { id: 'pe6', text: "Tendance 'Numérique Responsable'", cat: 'ENV', influence: { P: 3, E: 2, S: 5, T: 3, ENV: 5, L: 2 } },
        { id: 'pe7', text: "Stabilité politique de l'UE", cat: 'P', influence: { P: 5, E: 4, S: 3, T: 2, ENV: 2, L: 4 } },
        { id: 'pe8', text: "Augmentation des taux d'intérêt", cat: 'E', influence: { P: 3, E: 5, S: 2, T: 1, ENV: 1, L: 1 } },
        { id: 'pe9', text: "Vieillissement de la population", cat: 'S', influence: { P: 4, E: 3, S: 5, T: 2, ENV: 1, L: 2 } },
        { id: 'pe10', text: "Incitations fiscales R&D", cat: 'P', influence: { P: 5, E: 4, S: 1, T: 5, ENV: 2, L: 3 } },
        { id: 'pe11', text: "Raréfaction des terres rares", cat: 'ENV', influence: { P: 4, E: 5, S: 2, T: 4, ENV: 5, L: 1 } },
        { id: 'pe12', text: "Cyberattaques en forte hausse", cat: 'T', influence: { P: 4, E: 3, S: 4, T: 5, ENV: 1, L: 4 } }
    ];

    const poolContainer = document.getElementById('pestel-events-pool');
    const pestelColumns = document.querySelectorAll('.pestel-column');
    const pestelCounter = document.getElementById('pestel-counter');
    const pestelAnalysis = document.getElementById('pestel-analysis');
    const pestelVerdictText = document.getElementById('pestel-verdict-text');
    const pestelResetBtn = document.getElementById('pestel-reset');
    const pestelCompanySelect = document.getElementById('pestel-company');

    let placedEvents = 0;
    let pestelChart = null;
    let totalInfluence = { P: 0, E: 0, S: 0, T: 0, ENV: 0, L: 0 };

    function initPestel() {
        poolContainer.innerHTML = '';
        placedEvents = 0;
        totalInfluence = { P: 0, E: 0, S: 0, T: 0, ENV: 0, L: 0 };
        pestelCounter.textContent = `0 / ${pestelEvents.length} événements classés`;
        pestelAnalysis.classList.add('hidden');
        
        // Nettoyer les colonnes
        document.querySelectorAll('.drop-zone-pestel').forEach(zone => zone.innerHTML = '');

        // Mélanger les événements
        const shuffled = [...pestelEvents].sort(() => Math.random() - 0.5);

        shuffled.forEach(event => {
            const div = document.createElement('div');
            div.className = 'pestel-event';
            div.draggable = true;
            div.id = event.id;
            ...
            // Support Mobile (Click simple)
            div.addEventListener('click', () => {
                if (div.parentElement === poolContainer) {
                    // Sélectionner pour déplacement
                    document.querySelectorAll('.pestel-event').forEach(el => el.style.border = '');
    ...
    // Gestion du drop
    pestelColumns.forEach(column => {
        const zone = column.querySelector('.drop-zone-pestel');
        ...
        zone.addEventListener('drop', (e) => {
            ...
            handleEventDrop(id, category, zone);
        });

        // Support Mobile (Click sur zone)
        zone.addEventListener('click', () => {
            const selected = document.querySelector('.pestel-event[data-selected="true"]');
            if (selected) {
                handleEventDrop(selected.id, category, zone);
    ...
    function handleEventDrop(id, category, zone) {
        const event = pestelEvents.find(e => e.id === id);
        const element = document.getElementById(id);

        if (!event || !element || element.parentElement === zone) return;
        ...
        if (event.cat === category) {
            ...
            // Ajouter aux influences
            Object.keys(event.influence).forEach(k => {
                totalInfluence[k] += event.influence[k];
            });

            placedEvents++;
            pestelCounter.textContent = `${placedEvents} / ${pestelEvents.length} événements classés`;

            if (placedEvents === pestelEvents.length) {
                showFinalAnalysis();
            }
        } else {
            showToast('Mauvaise catégorie ! Réessaie.', 'danger');
    ...
    function showFinalAnalysis() {
        pestelAnalysis.classList.remove('hidden');
        updateRadarChart();

        const company = pestelCompanySelect.options[pestelCompanySelect.selectedIndex].text;
        
        // Trouver la force dominante
        const forces = ['P', 'E', 'S', 'T', 'ENV', 'L'];
        const maxForce = forces.reduce((a, b) => totalInfluence[a] > totalInfluence[b] ? a : b);
        ...
        pestelVerdictText.innerHTML = `
            <p>Analyse terminée pour <strong>${company}</strong>.</p>
            <p>La force qui influence le plus ton entreprise est la force <strong>${forceNames[maxForce]}</strong>.</p>
            <p>Cela signifie que ta stratégie doit prioriser l'adaptation aux évolutions de ce domaine pour rester compétitif et conforme.</p>
        `;

        // Scroll vers l'analyse
        pestelAnalysis.scrollIntoView({ behavior: 'smooth' });
    }
    ...
    function updateRadarChart() {
        ...
        const data = {
            labels: ['Politique', 'Économique', 'Sociologique', 'Technologique', 'Écologique', 'Légal'],
            datasets: [{
                label: 'Intensité de l\'influence',
                data: [
                    totalInfluence.P, 
                    totalInfluence.E, 
                    totalInfluence.S, 
        ...
        pestelChart = new Chart(ctx, config);
    }

    pestelResetBtn.addEventListener('click', initPestel);
    pestelCompanySelect.addEventListener('change', initPestel);

    // Initialisation au chargement
    initPestel();

    // ==========================================
    //  10. SIMULATEUR DE STRATÉGIE GLOBALE
    // ==========================================

    const stratScenarios = [
        {
            market: "Marché du Cloud Computing",
            desc: "Ta startup 'CloudNova' est un petit acteur sur le marché du stockage. Des géants dominent déjà, mais la demande explose.",
            choices: {
                specialisation: {
                    name: "Spécialisation",
                    desc: "Je ne fais que de la sécurité Cloud pour les professions médicales.",
                    pros: ["Expertise reconnue", "Forte valeur ajoutée", "Moins de concurrence directe"],
                    cons: ["Marché limité", "Risque d'arrivée d'un géant", "Dépendance à un seul secteur"]
                },
                diversification: {
                    name: "Diversification",
                    desc: "Je lance aussi une application de gestion de projets et un service de streaming pro.",
                    pros: ["Répartition des risques", "Synergies possibles", "Nouveaux revenus"],
                    cons: ["Dispersion des ressources", "Besoin de gros financements", "Manque d'image de marque"]
                },
                integration: {
                    name: "Intégration",
                    desc: "Je rachète mon fournisseur de serveurs pour maîtriser l'infrastructure.",
                    pros: ["Contrôle des coûts", "Sécurisation des approvisionnements", "Meilleure marge"],
                    cons: ["Coût de rachat élevé", "Métier différent à gérer", "Rigidité de la structure"]
                }
            }
        },
        {
            market: "Marché de la Cybersécurité",
            desc: "Tu diriges 'SecureMind'. Le marché est en pleine croissance mais très fragmenté avec beaucoup de petites solutions.",
            choices: {
                specialisation: {
                    name: "Spécialisation",
                    desc: "Focus total sur la détection des ransomwares par IA.",
                    pros: ["Innovation pointue", "Cible très précise", "Position de leader technologique"],
                    cons: ["Obsolescence rapide", "Rachat probable par un concurrent", "Equipe ultra-spécialisée difficile à recruter"]
                },
                diversification: {
                    name: "Diversification",
                    desc: "On propose aussi de la formation aux employés et de l'audit physique.",
                    pros: ["Offre complète '360'", "Fidélisation client", "Revenus récurrents"],
                    cons: ["Baisse de la qualité technique", "Complexité commerciale", "Besoin de profils variés"]
                },
                integration: {
                    name: "Intégration",
                    desc: "On développe notre propre processeur sécurisé (Intégration amont).",
                    pros: ["Avantage concurrentiel unique", "Indépendance technologique", "Barrière à l'entrée"],
                    cons: ["Investissements massifs en R&D", "Cycle de production long", "Risque industriel"]
                }
            }
        }
    ];

    const stratMarketTitle = document.getElementById('strat-market-title');
    const stratMarketDesc = document.getElementById('strat-market-desc');
    const stratNewScenarioBtn = document.getElementById('strat-new-scenario');
    const stratCards = document.querySelectorAll('.strat-card');
    const stratFeedback = document.getElementById('strat-feedback');
    const stratChoiceName = document.getElementById('strat-choice-name');
    const stratChoiceDesc = document.getElementById('strat-choice-desc');
    const stratProsList = document.getElementById('strat-pros-list');
    const stratConsList = document.getElementById('strat-cons-list');

    let currentScenarioIndex = 0;

    function initStrategy() {
        const scenario = stratScenarios[currentScenarioIndex];
        stratMarketTitle.textContent = scenario.market;
        stratMarketDesc.textContent = scenario.desc;
        
        // Cacher le feedback et désactiver les cartes
        stratFeedback.classList.add('hidden');
        stratCards.forEach(c => c.classList.remove('active'));
    }
    ...
    stratCards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            const scenario = stratScenarios[currentScenarioIndex];
            const choice = scenario.choices[choiceKey];
            ...
            stratChoiceName.textContent = `Choix : ${choice.name}`;
            stratChoiceDesc.textContent = `"${choice.desc}"`;
            
            stratProsList.innerHTML = choice.pros.map(p => `<li>${p}</li>`).join('');
            stratConsList.innerHTML = choice.cons.map(c => `<li>${c}</li>`).join('');

            stratFeedback.classList.remove('hidden');
            
            // Notification visuelle
            showToast(`Choix ${choice.name} analysé !`);
        });
    });

    stratNewScenarioBtn.addEventListener('click', () => {
        currentScenarioIndex = (currentScenarioIndex + 1) % stratScenarios.length;
        initStrategy();
        showToast('Nouveau scénario chargé');
    });
